        ...
        case WM_SETCURSOR:
         ...
         case WM_MOUSEMOVE:
            if( m_bActive && m_pd3dDevice != NULL )
            {
                ...
                // XXX/GWS: I commented out this line
				//if( !m_bWindowed )
                    ScreenToClient( m_hWnd, &m_cursorPosition);
                m_pd3dDevice->SetCursorPosition( m_cursorPosition.x, m_cursorPosition.y, 0 );
            }
            break;
...
void CD3DApplication::BuildPresentParamsFromSettings()
{
	...
	// XXX/GWS: Added for a lockable back buffer
	m_d3dpp.Flags = m_d3dpp.Flags | D3DPRESENTFLAG_LOCKABLE_BACKBUFFER;
}
...
void CD3DApplication::UpdateStats()
{
    ...
    if( fTime - fLastTime > 1.0f )
    {
        ...
        _sntprintf( m_strFrameStats, cchMaxFrameStats, _T("%.02f fps (%dx%d), %s%s%s, (%d,%d)"), m_fFPS,
                    m_d3dsdBackBuffer.Width, m_d3dsdBackBuffer.Height,
                    strFmt, strDepthFmt, pstrMultiSample, m_cursorPosition.x, m_cursorPosition.y );
